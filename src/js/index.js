(function () {
  ...
  function init() {
    ...
    DOM.bip49coin.on("input", calcForDerivationPath);
    DOM.bip49account.on("input", calcForDerivationPath);
    DOM.bip49change.on("input", calcForDerivationPath);
    DOM.bip84coin.on("input", calcForDerivationPath);
  ...
  function TableRow(index, isLast) {
    ...
    function calculateValues() {
      setTimeout(function () {
        ...
        if (networks[DOM.network.val()].name == "AIOZ - AIOZ Network") {
          const hrp = "aioz";
          // AIOZ uses Ethermint (EVM-compatible) address derivation
          address = EthermintBufferToAddress(keyPair.getPublicKeyBuffer(), hrp);
          pubkey = EthermintBufferToHexAddress(keyPair.getPublicKeyBuffer());
          privkey = libs.ethUtil.bufferToHex(keyPair.d.toBuffer(32));
        }
  ...
  var networks = [
    ...
    {
      name: "AIOZ - AIOZ Network",
      onSelect: function () {
        network = libs.bitcoin.networks.bitcoin;
        // Allow user to customize Coin Type - remove readonly
        DOM.bip44coin.prop("readonly", false);
        DOM.bip49coin.prop("readonly", false);
        DOM.bip84coin.prop("readonly", false);
      },
    },
    {
      name: "AUR - Auroracoin",
